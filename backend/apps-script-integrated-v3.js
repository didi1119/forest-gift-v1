// ===== Google Apps Script 佣金管理系統 v3.0 - 整合版 =====
...
function handleUseAccommodationPoints(data) {
  try {
    ...
    // 檢查可用點數（嚴格檢查，防止負數）
    const currentPoints = Math.max(0, partner.available_points || 0);
    ...
    updateRecord('Partners', partner.partner_code, {
      available_points: Math.max(0, (partner.available_points || 0) - deductAmount),
      points_used: (partner.points_used || 0) + deductAmount
    });
...
function handleConvertPointsToCash(data) {
  try {
    ...
    const newPointsUsed = (partner.points_used || 0) + convertAmount;
    const newPendingCommission = (partner.pending_commission || 0) + cashAmount;
