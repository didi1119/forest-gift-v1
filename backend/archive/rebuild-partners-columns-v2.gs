// ===== Google Apps Script - 重建 Partners 表欄位 V2 =====
// 這個腳本會：
// 1. 備份現有資料
// 2. 重建標準欄位結構
// 3. 遷移資料到新欄位
...
// 使用不同的變數名稱避免衝突
const REBUILD_SHEETS_ID = '1buMGx7T1SFnOIygylkqQURUDFsHGidXcQ-k3kx3Xmn4';

// 標準欄位定義
const PARTNERS_STANDARD_COLUMNS = [
  // 核心識別
  'id',
  'partner_code',
  'partner_name',
...
function rebuildPartnersTable() {
  try {
    const spreadsheet = SpreadsheetApp.openById(REBUILD_SHEETS_ID);
    ...
    const backupData = backupPartnersData(sheet);
    Logger.log(`備份了 ${backupData.length} 筆資料`);
    
    // 步驟 2: 清空表格並重建欄位
    Logger.log('步驟 2: 重建欄位結構...');
    rebuildPartnersColumns(sheet);
    
    // 步驟 3: 遷移資料
    Logger.log('步驟 3: 遷移資料到新欄位...');
    const migratedCount = migratePartnersData(sheet, backupData);
    Logger.log(`成功遷移 ${migratedCount} 筆資料`);
    
    // 步驟 4: 設置格式
    Logger.log('步驟 4: 設置表格格式...');
    formatPartnersSheet(sheet);
 ...
 * 備份現有資料
 */
function backupPartnersData(sheet) {
 ...
 * 重建欄位結構
 */
function rebuildPartnersColumns(sheet) {
  // 清空表格
  sheet.clear();
  
  // 設置新的表頭
  const headerRange = sheet.getRange(1, 1, 1, PARTNERS_STANDARD_COLUMNS.length);
  headerRange.setValues([PARTNERS_STANDARD_COLUMNS]);
  ...
  // 設置欄位寬度
  PARTNERS_STANDARD_COLUMNS.forEach((column, index) => {
    let width = 100; // 預設寬度
 ...
 * 遷移資料到新欄位
 */
function migratePartnersData(sheet, backupData) {
  ...
  backupData.forEach((oldRow, index) => {
    ...
    PARTNERS_STANDARD_COLUMNS.forEach(column => {
      let value = '';
      ...
      // 檢查舊欄位名稱
        ...
        // 反向查找映射
        for (const [oldField, newField] of Object.entries(PARTNERS_FIELD_MAPPING)) {
      ...
      if (value === '' || value === undefined || value === null) {
        switch(column) {
          ...
          case 'successful_referrals':
            value = parseFloat(oldRow.total_successful_referrals) || 0;
            break;
  ...
  if (migratedData.length > 0) {
    const dataRange = sheet.getRange(2, 1, migratedData.length, PARTNERS_STANDARD_COLUMNS.length);
 ...
 * 設置表格格式
 */
function formatPartnersSheet(sheet) {
  const lastRow = sheet.getLastRow();
  
  if (lastRow > 1) {
    // 設置資料區域格式
    const dataRange = sheet.getRange(2, 1, lastRow - 1, PARTNERS_STANDARD_COLUMNS.length);
    ...
    numberColumns.forEach(column => {
      const columnIndex = PARTNERS_STANDARD_COLUMNS.indexOf(column) + 1;
 ...
 * 檢查現有欄位（診斷用）
 */
function checkPartnersStructure() {
  try {
    const spreadsheet = SpreadsheetApp.openById(REBUILD_SHEETS_ID);
    const sheet = spreadsheet.getSheetByName('Partners');
    ...
    report += '現有欄位:\n';
    headers.forEach((header, index) => {
      const isStandard = PARTNERS_STANDARD_COLUMNS.includes(header);
    ...
    headers.forEach(header => {
      if (header && !PARTNERS_STANDARD_COLUMNS.includes(header)) {
        report += `- ${header}\n`;
      }
    });
 ...
 * 只檢查不重建
 */
function runPartnersCheck() {
  return checkPartnersStructure();
}